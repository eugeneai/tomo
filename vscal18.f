...
C---------------------------------------------
     ...
     #            RCC      =0.2,  ! radius central condactor 
...
c      OPEN(36,   FILE='model_rec.gnu')
      ...
      OPEN(54,   FILE='phaze_exc_x.gnu')
      OPEN(55,   FILE='phaze_rec_x.gnu')


      OPEN(56,   FILE='phaze_exc_y.gnu')
      OPEN(57,   FILE='phaze_rec_y.gnu')
...
c      N1=4
c      JJ=1
c      CALL IFITET(JJ,N1,N2,I1,I2)

c      WRITE(*,*) 'I1=',I1,'I2=',I2

c      CALL IALBET(JJ,N1,I1,I2)
...
c----------------------------
c      IF(INFNOT) 2,4,6 
c 2    CALL Summa_Harm_Vec(LL,NNMAX,RRM,TETAM,PHIM,
c     #                            NRR,NTETA,NPHI,XM,GM)
c      GOTO 8
c 4    CALL Model_Infty2(NNMAX,RRM,TETAM,PHIM,
c     #                            NRR,NTETA,NPHI,GM)
c      GOTO 8

      CALL Model_Solov_2(ZZM,YYM,XXM,NZ,NY,NX,DR)
 8    CONTINUE
...
c-------------------
ccccc
cc      CALL Summa_Scal_Harm(LL,NNMAX,RRM,TETAM,PHIM,
cc     #                    NRR,NTETA,NPHI,XM,GM)


cc      CALL POLAR_DECART_VEC(GM,XXM,YYM,ZZM,NX,NY,NZ,RRM,TETAM,PHIM,
cc     %                  NRR,NTETA,NPHI,DR)
c------------
      CALL POLAR_DECART(GM,XXM,YYM,ZZM,NX,NY,NZ,RRM,TETAM,PHIM,
     %                  NRR,NTETA,NPHI,DR)
c----------------
cc  proveryal dla SETa v pryamougolnoi sisteme coordinat 
cc      CALL Model_Infty3(NNMAX,XXM,YYM,ZZM,
cc     #                            NX,NY,NZ,DR)


cc      CALL TORUS(XXM,YYM,ZZM,NX,NY,NZ,PHIM,NPHI,DR,PAR)


cc      CALL Model_Solov(ZZM,RRM,PHIM,NZ,NRR,NPHI,DR)


cc      CALL PSIFUNM(XXM,ZZM,NX,NZ,PSIM)
c      CALL PSIFUNM(ZZM,RRM,NZ,NRR,PSIM)

cc      CALL GNUFORM(NZ,NX,1,PSIM,17)
c      CALL GNUFORM_X_Y (ZZM,RRM,NZ,NX,PSIM,17)      
      ...
      CALL GNUFORMVEC_Z (XXM,YYM,NX,NY,NZ,KNZ,DR,46,58)

      CALL GNUFORMVEC_Y (XXM,ZZM,NX,NY,NZ,KNY,DR,47,56)

      CALL GNUFORMVEC_X (YYM,ZZM,NX,NY,NZ,KNX,DR,51,54)
c--------------------------------------------------
c      return
c 222  continue
...
cc     %         XXM,YYM,ZZM,NPAR,PAR,IERR)
      ...
      DO 11 I4=1,NPP
      DO 11 II=KJ,KJ  ! 1,NJ
         CALL IALBET(II,NALPHA,I1,I2)
         ALPHA=ALPHAM(I1)
         BETA =BETAM(I2)
c         GAMMA=GAMMAM(I3)

         I321=(I2-1)*NALPHA+I1
         I4321=(I4-1)*NBETA*NALPHA + I321
         PROJ2(I4)=PROJ(I4321)
 11   CONTINUE   

      write(*,*) 'alpha_n=',ALPHA*180/PI
      write(*,*) 'beta_n=',BETA*180/PI
c      write(*,*) 'gamma_n=',GAMMA*180/PI
...
c      CALL NORMER3(DR13,DR23,NX,NY,NZ,SVZKA3)
      ...
      CALL GNUFORMVEC_Z (XXM,YYM,NX,NY,NZ,KNZ,DDR,48,59)

      CALL GNUFORMVEC_Y (XXM,ZZM,NX,NY,NZ,KNY,DDR,49,57)

      CALL GNUFORMVEC_X (YYM,ZZM,NX,NY,NZ,KNX,DDR,52,55)
...
c 2    CONTINUE   
      ...
      DO 2 J=1,NZ
         ZZ=ZZM(J)
         IJ=(I-1)*NZ+J
         PSIM(IJ)=PSIFUN(RR,ZZ)
 2    CONTINUE   

      RETURN
      END

      FUNCTION PSIFUN(RRR,ZZ)
...
c      PSIFUN=PSI0*(RR-R0)**2*(2*R0**2-(RR-R0)**2-4*AL*ZZ**2)/R0**4
      ...
      SUBROUTINE Model_Solov_2(ZZM,YYM,XXM,NZ,NY,NX,GM)
c
...
c      write(*,*) 'I1=',I1,'I2=',I2,'I3=',I3
 16   CONTINUE
      ...
      SUBROUTINE Model_Solov(ZZM,RRM,PHIM,NZ,NRR,NPHI,GM)
c
      REAL RRM(NRR),PHIM(NPHI),ZZM(NZ)
      REAL GM(NRR*NZ*NPHI,3)
...
c            I23=(I2-1)*NRR+I3
c            I321=(I1-1)*NRR*NZ+(I2-1)*NRR+I3

            GR=1./(2*PI*RR)*8*PSI0*AL/RR0**4*RR**2*ZZ
c              GR=4./(PI*RR0**4)*PSI0*AL*RR*ZZ
            GZ=1./(2*PI*RR)*(-4*PSI0*RR**3/RR0**4+
     #           2*RR*(2*PSI0/RR0**2-4*AL*PSI0*ZZ**2/RR0**4))
...
c      CC1=COS(M*ALPHA + M1*GAMMA)
c      CC2=COS(M*ALPHA - M1*GAMMA)

      SS1=SIN(M*ALPHA)
      SS2=SIN(M*ALPHA)

c      SS1= SIN(M*ALPHA + M1*GAMMA)
c      SS2= SIN(M*ALPHA - M1*GAMMA)
...
c         WUnit(1)=-SIN(BETA)*COS(GAMMA)
c         WUnit(2)= SIN(BETA)*SIN(GAMMA)
c         WUnit(3)= COS(BETA)
...
c         WUnit(3)= COS(BETA)
 ...
 16   CONTINUE
      ...
      DO 18 I2=1,NBETA
         ...
         WW2=1.0
         IF(I2 .EQ.1 .OR. I2 .EQ. NBETA) WW2=0.5
         S1=S1+BM1(I2,1)*SIN(BETA)*WW2
         S2=S2+BM1(I2,2)*SIN(BETA)*WW2
         S3=S3+BM1(I2,3)*SIN(BETA)*WW2
...
c         I23=(I2-1)*NGAMMA+I3

c         WW3=1.0
c         IF(I3 .EQ.1 .OR. I3 .EQ. NGAMMA) WW3=0.5
c         S1=S1+BM1(I23,1)*WW3
c         S2=S2+BM1(I23,2)*WW3
c         S3=S3+BM1(I23,3)*WW3
c 24   CONTINUE
c         BM2(I2,1)=S1*HGAMMA
c         BM2(I2,2)=S2*HGAMMA
c         BM2(I2,3)=S3*HGAMMA
c 26   CONTINUE
...
c         CALL IALBETGAM(I4,NALPHA,NBETA,I1,I2,I3)
         ALPHA=ALPHAM(I1)
         BETA =BETAM(I2)
c         GAMMA=GAMMAM(I3)
c         I321=(I3-1)*NBETA*NALPHA+(I2-1)*NALPHA+I1
         I321=(I2-1)*NALPHA+I1
...
c         AA33= CBETA
         ...
         WUnit(2)= 0.
         WUnit(3)= CBETA
c         WUnit(1)=-SBETA*CGAMMA
c         WUnit(2)= SBETA*SGAMMA
c         WUnit(3)= CBETA
...
c Вычисление функции U(lmn)  формула (20)
      KK=MOD(L+M,2)
...
c 
      ...
      DO 6 I4=1,NPP
         PP=PPM(I4)
c         I321=(I3-1)*NBETA*NALPHA+(I2-1)*NALPHA+I1
c         I4321=(I4-1)*NGAMMA*NBETA*NALPHA + I321
         I321=(I2-1)*NALPHA+I1
         I4321=(I4-1)*NBETA*NALPHA + I321
         PROJ2(I4)=PROJ(I4321)
 6    CONTINUE   
...
c      NKAPPA=NPAR(20)
      ...
      BETA0=0.
      BETA1=PI
c      HBETA=(BETA1-BETA0)/(NBETA-1)
      HBETA=(BETA1-BETA0)/NBETA
...
c 10   CONTINUE
      ...
      ALPHA0=-PI
      ALPHA1= PI
c      HALPHA=(ALPHA1-ALPHA0)/(NALPHA-1)
      HALPHA=(ALPHA1-ALPHA0)/NALPHA

      GAMMA0=0.
      GAMMA1=2.*PI
c      HGAMMA=(GAMMA1-GAMMA0)/(NGAMMA-1)
      HGAMMA=(GAMMA1-GAMMA0)/NGAMMA
...
C calculation I1, I2-- tet and fi indexes by number of projection JJ
C  JJ=(I2-1)*N1+I1
C
      MODUL=MOD(JJ,N1)
      IC=1
...
C calculation I1, I2, --  indexes by the number of  JJ,
C where JJ is calculated as   JJ=(I2-1)*N1+I1
