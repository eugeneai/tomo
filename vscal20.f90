...
!        YM((LL+1)*(LL+1)*NNMAX,NTEN*NTEN)
        ALLOCATE (PAR(50), PPM(NPP), RRM(NRR),  &       
                  ...
                  XM(NX),YM(NY),ZM(NZ),        &
                  UM(NU),VM(NV),WM(NW),        &
                  XXM(LN,NTEN),XXM2(LN,NTEN),XXM0(LN),XXM1(LN), &     
                  YMVEC(LN,NTEN), YMVEC3(LN*NTEN),   &
!                  XMVEC(NTEN*NTEN*LN),XMVEC0(NTEN*LN),XMVEC1(NTEN*LN), &

                  AMVEC(LN,LN), AMVEC3(NTEN*LN,NTEN*LN),     &
                  XMVEC(NTEN*LN),XMVEC0(NTEN*LN),   &
                  XMVEC1(NTEN*LN),&

                  GMRE(NRR*NTETA*NPHI,NTEN),            &
                  DGMRE(NX*NY*NZ,NTEN),      &
...
!=================================================================
      ...
      CALL GNUFORM_0 (NTEN*LN,XMVEC,KAN(30))   !! plot cof_exc.gnu
 
     write(*,*) 'solution exact XMVEC'
      write(*,117) (XMVEC(I), I=1, NTEN*LN)
...
!!не забыть поменял PPM на UM
       CALL ProjNum_TEN(DGMRE,PRJ3DIM,UM,  &
              ALPHAM,BETAM,GAMMAM,NALPHA,NBETA,NGAMMA, &
              XM,YM,ZM,NX,NY,NZ,NPP,NW,NTEN)


!!не забыть поменял PPM на UM
       CALL Proj_Summa_W_Harm(LL,NJ,NNMAX,UM,ALPHAM,BETAM, &
              GAMMAM,NPP,NALPHA,NBETA,NGAMMA,XXM,NTEN,PRJ3DRE)


!       write(*,*) '11111111',LL,NJ,NNMAX,NPP,NALPHA,NBETA,NGAMMA,NTEN
...
!          CALL GNUFORM(NPP,1,PROJIM1D,KAN(44)) 
!          CALL GNUFORM(NPP,1,PROJRE1D,KAN(45)) 
!                  SUBROUTINE GNUFORM (NX,NY,GC,KAN)

          write(*,*) 'proj2',(REAL(PROJIM1D(I)),I=1,NPP)
...
!!не забыть поменял PPM на UM
          ...
          write(*,117) (YMVEC3(I), I=1, NTEN*LN)
...
!!не забыть поменял PPM на UM
      CALL Matrix_WW_TEN(LL,NNMAX,PPM,UM,ALPHAM,BETAM,GAMMAM, &
                     NPP,NALPHA,NBETA,NGAMMA,AMVEC,PAR,NTEN,NII,NJJ)


!!     CALL Sort_Matr(AMVEC,LN,AMVEC3,NTEN,NNMAX,NII,NJJ)
      CALL Sort_Matr(AMVEC,LL,LN,AMVEC3,NTEN,NNMAX,NII,NJJ)

 81   CONTINUE

      write(*,*) 'Matrix AMVEC3'
      DO 97 I=1,NTEN*LN
      write(*,117) (AMVEC3(I,J), J=1, NTEN*LN)
 97   CONTINUE
!-----------------------------------------------
...
! initial values for the solution
 ...
 13   CONTINUE 
      ...
      CALL XYANG_1(AMVEC3,YMVEC3,XMVEC0,NTEN*LN,NTEN*LN,NITER)
!... am(n1,n2) - matrix 
!....gm(n1) - right part 
...
!      CALL Vec_Line_Inv(XMVEC0,XXM2,LN,NTEN)
      ...
      write(*,117) (XMVEC0(I), I=1, NTEN*LN)

      CALL GNUFORM_0 (NTEN*LN,XMVEC0,KAN(31)) 
...
!====================================================
      ...
      SUBROUTINE RightPart_Full(LL,NNMAX,NTEN,     &
        ...
        REAL(wp) :: YMM((LL+1)**2*NNMAX*NTEN**2)
...
!local
        ...
        DO 30 M=0,L   
           ...
           CALL RightPart_Term(PROJ,LL,NNMAX,NJ,  &
...
!           TERM = Yarray_Term(   )
           ...
           DO 20 I=1,NTEN*NTEN
              MU_W=LMAX*((I-1)*NNMAX+N-1)+L*L+M+1
              IF(M .NE. 0) THEN 
              MU_V=LMAX*((I-1)*NNMAX+N-1)+L*L+L+M+1
              ENDIF
              YMM(MU_W)=YM(1,I)
              IF(M .NE. 0) THEN 
              YMM(MU_V)=YM(2,I)
...
!--------------------------------------------------
     ...
     SUBROUTINE RightPart_Term(PROJ,LL,NNMAX,NJ,  &
                PPM,ALPHAM,BETAM,GAMMAM,        &
                NPP,NALPHA,NBETA,NGAMMA,         &
                NTEN,PAR,YM)
        ...
        INTEGER :: LL,NNMAX,NJ,NPP,NTEN,NPAR
        INTEGER :: NALPHA,NBETA,NGAMMA
        REAL(wp) :: PROJ(NPP*NALPHA*NBETA*NGAMMA)
...
!local 
        ...
        REAL(wp) :: S2(NTEN,NTEN),S3,WW1,WW2,WW3
...
!                             NJ,I1,I2,I3,PAR,YYRE,YYIM)
            ...
            DO 4 I=1,NTEN   
               IJ=(J-1)*NTEN+I
               S2(I,J)=S2(I,J)+YYRE*WUnit(I)*WUnit(J)*WW1
4           CONTINUE
...
!         CALL INT_Proj_GNLM_P(PROJ,PPM,NPP,         &
!                     N,L22,M,                          & 
!                     ALPHA,BETA,GAMMA,                   &
!                     I1,I2,I3,NJ,PAR,YYRE,YYIM)
         
         DO 41 J=1,NTEN
         DO 41 I=1,NTEN   
            IJ=(J-1)*NTEN+I
            S2(I,J)=S2(I,J)+YYIM*WUnit(I)*WUnit(J)*WW1
41       CONTINUE
20    CONTINUE
...
28    CONTINUE
         ...
         DO 191 I=1,NTEN   
            IJ=(J-1)*NTEN+I
            YM(2,IJ)=S2(I,J)*HBETA
191       CONTINUE
!50    CONTINUE
      RETURN
      END SUBROUTINE RightPart_Term
!--------------------------------------
...
!------------------------------------------------- 
      ...
      SUBROUTINE Matrix_WW_TEN(LL,NNMAX,PPM,RRM,ALPHAM,BETAM,GAMMAM, &
                     NPP,NALPHA,NBETA,NGAMMA,AM,PAR,NTEN,NI,NJ)
...
!     write(*,*) 'Matrix_WW_TEN_C_C', IIC,IIC1,L11,L22
 20   CONTINUE
 30   CONTINUE
...
!        write(*,*) 'Matrix_W: 50 L1=  ',L1
 ...
 !       write(*,*) 'Matrix_WW_TEN_S_S', IIS,IIS1,L11,L22
 40   CONTINUE
 50   CONTINUE
!-----------------------------------------------
...
!  cos * sin  (1,2)
 ...
 !        write(*,*) 'Matrix_WW_TEN_C_S', IIC1,IIS,L11,L22
 60   CONTINUE
 70   CONTINUE
!------------------------------------------
...
!  sin * cos   (2,1)
      ...
      CALL WW_WW_TEN(L11,M1,N1,L22,M-1,N,       &
         ...
         AM(IIC,IIS1)=VLM(2)
 !        write(*,*) 'Matrix_WW_TEN_S_C', IIS1,IIC,L11,L22
...
!                     HUPP,GG12)
      GG12= INT_GNLMP_GNLMP(N1,L1,K1,N2,L2,K2,UPPM,NPP,HUPP)
...
!--------------------------------------------------------
!      FUNCTION Submatrix(L1,M1,N1,L2,M2,N2,ALPHA,BETA,GAMMA,NI,NJ) 

      FUNCTION WLMM_RE_TEN(L,M,N,ALPHA,BETA,GAMMA,PP) 
...
! local
        ...
        REAL(wp) :: VLMRE,GN,S
! functions
        REAL(wp) :: PLM1M2,WLMM_RE_TEN,GNLMP 
        ...
        DO 4 I=1,L
           ...
           CC1=COS(M*ALPHA + K*GAMMA)
           CC2=COS(M*ALPHA - K*GAMMA)
...
!      CALL INT_GNLMP(PPM,UPPM,NPP,N1,L1,K1,N2,L2,K2,  &
!                     HUPP,GG12)
      GG12= INT_GNLMP_GNLMP(N1,L1,M1,N2,L2,M2,UPPM,NPP,HUPP)
...
!--------------------------------------
      ...
      FUNCTION INT_GNLMP_GNLMP(N,L,M,N1,L1,M1,PPM,NPP,HPP)
! Integration of U_nlm(P)*U_n1l1m1(P) over P variable
        IMPLICIT NONE
        INTEGER, PARAMETER  :: wp = kind(1.0d0) !working precision=double 
        INTEGER, PARAMETER  :: sp = kind(1.0) !working precision=single 
...
!local 
        ...
        REAL(wp) :: S,PP,WW1
!functions
        REAL(wp) ::  INT_GNLMP_GNLMP,GNLMP_GNLMP
      ...
      DO 8 I1=1,NPP
         ...
         S=S+GNLMP_GNLMP(N,L,M,N1,L1,M1,PP)*WW1
 8    CONTINUE   
...
!------------------------------------------
      FUNCTION GNLMP_GNLMP(N,L,M,N1,L1,M1,PP)
!  произведение двух функции U(lmn)  (формула (20)
! статья Мат. Мод. 2005) 
...
!functions
        REAL(wp) :: GNLMP,GNLMP_GNLMP
        
        GNLMP_GNLMP=GNLMP(N,L,M,PP)*GNLMP(N1,L1,M1,PP)

      RETURN
      END FUNCTION GNLMP_GNLMP
!------------------------------------------
...
!  summation of W  harmonics 
        ...
        REAL(wp) :: ALPHAM(NALPHA),BETAM(NBETA),GAMMAM(NGAMMA)
        REAL(wp) :: PPM(NPP)
        REAL(wp) :: XXM((LL+1)*(LL+1)*NNMAX,NTEN)
...
!           write(*,*) 'Proj_Summa_W_Harm',WWLMRE,WWLMIM
...
4       CONTINUE
        ...
        DO 6 M=1,L22
           ...
           CALL WWCS(N,L22,M,ALPHA,BETA,GAMMA,PP,WWLMRE,WWLMIM)
!           write(*,*) 'Proj_Summa_W_Harm',WWLMRE,WWLMIM
!c           CALL YLMRE (L,M,TETA,PHI,YLMRE,YLMIM)
...
!      SUBROUTINE ProjNum_TEN(DR,PROJ,UM,PPM,WM,  &
!              ALPHAM,BETAM,GAMMAM,NALPHA,NBETA,NGAMMA, &
!              XM,YM,ZM,NX,NY,NZ,NU,NPP,NW,NTEN)
...
!local 
        REAL(wp) :: DDR1(NX*NY*NZ,NTEN/3), DDR(NX*NY*NZ)
        REAL(wp) :: PROJ2(NPP)
        REAL(wp) :: WUnit(3)
        REAL(wp) :: ALPHA,BETA,GAMMA 
...
!  summation of scalar spherical harmonics 
        ...
        REAL(wp) :: GM(NRR*NTETA*NPHI,NTEN)
        REAL(wp) :: XXM((LL+1)*(LL+1)*NNMAX,NTEN)
!local
