...
cc     #                            NRR,NTETA,NPHI,GM)
     ...
     #                            NRR,NTETA,NPHI,XM,GM)
      ELSE
         CALL Model_Infty2(NNMAX,RRM,TETAM,PHIM,
     #                            NRR,NTETA,NPHI,GM)
      ENDIF   
...
c  proveryal dla SETa v pramougolnoi sisteme coordinat 
c      CALL Model_Infty3(NNMAX,XXM,YYM,ZZM,
c     #                            NX,NY,NZ,DR)
...
!      CALL NORMER2(DR13,DR23,NX,NY,SVZKA3)
 ...
 666  continue
      ...
      CALL RightPart_WW(PROJ,LL,NJ,NNMAX,
     #           PPM,UPPM,ALPHAM,BETAM,GAMMAM,
     #           NPP,NALPHA,NBETA,NGAMMA,
     #           NPAR,PAR,YM)
       ...
       write(*,*) 'right part_ww'
      write(*,117) (YM(I), I=1, LN)
c-------------------------------------------------

      CALL Matrix_WW(LL,NNMAX,PPM,UPPM,ALPHAM,BETAM,GAMMAM,
     #       NPP,NALPHA,NBETA,NGAMMA,NPAR,PAR,AM)

      write(*,*) 'Matrix1111 AM'
      DO 98 I=1,LN
      write(*,117) (AM(I,J), J=1, LN)
 98   CONTINUE
...
c initial values for the solution
 ...
 10   CONTINUE 
      ...
      CALL XYANG(AM,YM,XM0,XM1,A1M,LN,LN,NITER)
      WRITE(IPRNT,*) 'solution of system'       
      WRITE(IPRNT,117) (XM1(I),I=1,LN) 
      CALL GNUFORM1 (LN,XM1,22)   !! plot sol_rec.gnu

c-----------------------------------------------


      CALL Summa_Scal_Harm(LL,NNMAX,RRM,TETAM,PHIM,
     #                    NRR,NTETA,NPHI,XM1,GGM)

      CALL POLAR_DECART(GGM,XXM,YYM,ZZM,NX,NY,NZ,RRM,TETAM,PHIM,
     %                  NRR,NTETA,NPHI,DDR)

      CALL SPRINT(DDR,DR1,XXM,YYM,ZZM,NX,NY,NZ,36)
...
c         XX=XM(I1)
      ...
      WRITE(KAN1,100) YY,ZZ,GC(I123,1),GC(I123,2)
      WRITE(KAN1,100)
10    CONTINUE 
...
c         YY=YM(I2)
      ...
      WRITE(KAN1,100) XX,ZZ,GC(I123,1),GC(I123,2)
      WRITE(KAN1,100)
10    CONTINUE 
...
110   FORMAT(E10.3)
      ...
      WRITE(KAN1,100) XX,YY,GC(I123,1),GC(I123,2)
      WRITE(KAN1,100)
10    CONTINUE 
...
c         VM(I)=PPM(I)   !  VM nije zacomentirival
