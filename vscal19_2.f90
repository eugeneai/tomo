...
!        YM((LL+1)*(LL+1)*NNMAX,NTEN*NTEN)
        ALLOCATE (PAR(50), PPM(NPP), RRM(NRR),  &       
                  ...
                  XM(NX),YM(NY),ZM(NZ),        &
                  UM(NU),VM(NV),WM(NW),        &
                  XXM(LN,NTEN*NTEN),XXM2(LN,NTEN),XXM0(LN),XXM1(LN), &     
                  YMVEC(LN,NTEN*NTEN),    &
...
!=================================================================
      ...
      CALL GNUFORM_0 (NTEN*NTEN*LN,XMVEC,30)   !! plot cof_exc.gnu
...
!          SDELTA=8.*PI**2
!          write(*,*) '11111111',YMVEC
!          write(*,*) '22222222211111111',SDELTA
...
!--------------------------------------------------------
      SUBROUTINE INT_VLMM_TEN(L1,M1,K1,L2,M2,K2,ALPHAM,BETAM,GAMMAM, &
        NBETA,NGAMMA,NALPHA,NPAR,PAR,VLMINT,NI1,NI2,NJ1,NJ2)
...
!
        ...
        REAL(wp) :: PROJ(NV*NALPHA*NBETA*NGAMMA)
...
! local
        ...
        AACOS(1)=1.
        IF(L .EQ.0 .AND. M .EQ.0) AACOS(1)=-1.

        AACOS(2)=0.   
        IF(L .EQ.0 .AND. M .EQ.0) AACOS(2)=0.5     
        ...
        AACOS(5)=1.   
        IF(L .EQ.0 .AND. M .EQ.0) AACOS(5)=-1. 
        ...
        AACOS(8)=0.   
        IF(L .EQ.0 .AND. M .EQ.0) AACOS(8)=0.5 

        AACOS(9)=1.   
        IF(L .EQ.0 .AND. M .EQ.0) AACOS(9)=-1. 
