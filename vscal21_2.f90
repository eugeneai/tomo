...
!              S1=S1+XXM(IIC,IJ)*WWLMRE*WUnit(I)*WUnit(J)
              WWLMRE=WWC(I,J,N,L22,M-1,ALPHA,BETA,GAMMA,PP)
...
!           write(*,*) 'Proj_Summa_W_Harm:IMAGE',WWLMIM
           ...
           DO 9 I=1,NTEN   
              IJ=(J-1)*NTEN+I
              WWLMIM= VVS(I,J,N,L22,M,ALPHA,BETA,GAMMA,PP)
...
!------------------------------------------------------
      ...
      SUBROUTINE RightPart_Full(LL,NNMAX,NTEN,     &
        ...
        REAL(wp) :: PAR(50)
!local
        INTEGER :: IC,JC,N,L,M,MU_W,MU_V,I
...
!cos
        ...
        DO 30  M=1,L22+1
           ...
           CALL RightPart_Term(PROJ,IC,JC,LL,N,L22,M-1,NNMAX,NJ,  &
...
!  sin
        ...
        DO 60  M=1,L22
           ...
           CALL RightPart_Term(PROJ,IC,JC,LL,N,L22,M,NNMAX,NJ,  &
                PPM,ALPHAM,BETAM,GAMMAM,        &
                NPP,NALPHA,NBETA,NGAMMA,             &
                NTEN,PAR,YM)
...
!--------------------------------------------------
     ...
     SUBROUTINE RightPart_Term(PROJ,IC,JC,LL,N,L,M,NNMAX,NJ,  &
                PPM,ALPHAM,BETAM,GAMMAM,        &
                NPP,NALPHA,NBETA,NGAMMA,         &
                NTEN,PAR,YM)
...
!        REAL(wp) :: S1(NTEN,NTEN),S2(NTEN,NTEN),WW1,WW2,WW3
        REAL(wp) :: S1,S2,WW1,WW2,WW3
        REAL(wp) :: ALPHA,BETA,GAMMA
        REAL(wp) :: HALPHA,HBETA,HGAMMA
        REAL(wp) :: YYRE,YYIM
!functions
        REAL(wp) :: INT_Proj_GNLM_RE, INT_Proj_GNLM_IM
...
!            DO 4 I=1,NTEN   
!               IJ=(J-1)*NTEN+I
!               S1(I,J)=S1(I,J)+YYRE*WUnit(I)*WUnit(J)*WW1
!               S2(I,J)=S2(I,J)+YYIM*WUnit(I)*WUnit(J)*WW1
!4           CONTINUE

!               S1=S1+YYRE*WUnit(IC)*WUnit(JC)*WW1
!               S2=S2+YYIM*WUnit(IC)*WUnit(JC)*WW1
...
!         DO 5 I=1,NTEN   
!            IJ=(J-1)*NTEN+I
!            BM1(I23,IJ)=S1(I,J)*HALPHA
!            BM2(I23,IJ)=S2(I,J)*HALPHA
!5        CONTINUE

!            BM1(I23,IJ)=S1(I,J)*HALPHA
!            BM2(I23,IJ)=S2(I,J)*HALPHA
            BM1(I23,IC,JC)=S1*HALPHA
            BM2(I23,IC,JC)=S2*HALPHA
...
!               DO 9 I=1,NTEN   
!                  IJ=(J-1)*NTEN+I
!                  S1(I,J)=S1(I,J)+BM1(I23,IJ)*WW3
!                  S2(I,J)=S2(I,J)+BM2(I23,IJ)*WW3
!9              CONTINUE

                  S1=S1+BM1(I23,IC,JC)*WW3
                  S2=S2+BM2(I23,IC,JC)*WW3
14          CONTINUE
!            DO 11 J=1,NTEN
!            DO 11 I=1,NTEN   
!               IJ=(J-1)*NTEN+I
!               CM1(I2,IJ)=S1(I,J)*HGAMMA
!               CM2(I2,IJ)=S2(I,J)*HGAMMA
!11          CONTINUE
...
!         DO 17 I=1,NTEN   
!            IJ=(J-1)*NTEN+I
!            S1(I,J)=S1(I,J)+CM1(I2,IJ)*SIN(BETA)*WW2
!            S2(I,J)=S2(I,J)+CM2(I2,IJ)*SIN(BETA)*WW2
!17       CONTINUE
            S1=S1+CM1(I2)*SIN(BETA)*WW2
            S2=S2+CM2(I2)*SIN(BETA)*WW2
18    CONTINUE

!         DO 19 J=1,NTEN
!         DO 19 I=1,NTEN   
!            IJ=(J-1)*NTEN+I
!            YM(1,IJ)=S1(I,J)*HBETA
...
! 8    CONTINUE   
      ...
      DO 10 I4=1,NPP
         ...
         WWLMRE= WWC(IC,JC,N1,L1,M1,ALPHA,BETA,GAMMA,PP)
...
!         WWLMRE= WWC(N1,L1,M1,ALPHA,BETA,GAMMA,PP)
         WWLMIM= VVS(IC,JC,N1,L1,M1,ALPHA,BETA,GAMMA,PP)
!         CALL WWCS(N1,L1,M1,ALPHA,BETA,GAMMA,UPP,WWLMRE,WWLMIM)
...
!------------------------------------------------------------
      ...
      FUNCTION WWC(IC,JC,N,L,M,ALPHA,BETA,GAMMA,PP)
...
10      CONTINUE
      ...
      END FUNCTION WWC
!------------------------------------------------
      FUNCTION VVS(IC,JC,N,L,M,ALPHA,BETA,GAMMA,PP)
...
2     CONTINUE
      ...
      DO 85 I4=1,NJ 
         ...
         PROJ(I4321)= ProjNum_FUN(DR,PP,ALPHA,BETA,GAMMA,   & 
                       XM,YM,ZM,NX,NY,NZ,NPP,NW,NTEN)
85    CONTINUE
      RETURN
...
!------------------------------------------
      ...
      FUNCTION ProjNum_FUN(DR,PP,ALPHA,BETA,GAMMA,   & 
                       XM,YM,ZM,NX,NY,NZ,NPP,NW,NTEN)
        USE PARAM_INPUT
...
!local
        ...
        INTEGER :: I
!functions
        REAL(wp) :: GNLMP_GNLMP, INT_GNLMP_GNLMP
