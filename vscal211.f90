...
!        REAL(wp), allocatable::SVZKARE(:),SVZKAIM(:)
        ...
        OPEN(10,FILE ='RightPart.dat', ERR=300)
...
!=================================================================
      ...
      CALL XXMM(LL,NNMAX,XXM,NTEN,NDIM)  
...
!  IND= 1  -- section YZ
      ...
      IND=0
      CALL SPRINT_3D(DGMRE,XM,YM,ZM,NX,NY,NZ,IND,KNZ,NTEN,1,KAN(23))
      IND=0
      CALL SPRINT_3D(DGMRE,XM,YM,ZM,NX,NY,NZ,IND,KNX,NTEN,1,KAN(20))
...
!          write(*,*) 'I1,I2,I3',I1,I2,I3
          ...
          CALL GNUFORM1(PPM,NPP,PROJIM1D,KAN(45)) 
          CALL GNUFORM1(PPM,NPP,PROJRE1D,KAN(44)) 
...
!       write(10,117) (YMVEC3(I), I=1, NTEN*NTEN*LN)
!       READ(10,117) (YMVEC3(I), I=1, NTEN*NTEN*LN)
...
!       CALL Matrix_2(LL,NNMAX,PPM,ALPHAM,BETAM,GAMMAM, &
!                     NPP,NALPHA,NBETA,NGAMMA,NTEN,PAR,AMVEC3)

!       write(*,*) 'Matrix :AMVEC3'
!       DO 14 I=1,NTEN*NTEN*LN
...
!       GOTO 777
      ...
      CALL GNUFORM_0 (NTEN**2*LN,XMVEC0,KAN(31)) 
...
!        WUnit(3)= COS(BETA)
      ...
      DO 10 I4=1,NPP
         ...
         WWLM1= WWC(IC1,JC1,N1,L1,M1,ALPHA,BETA,GAMMA,PP)
         WWLM2= WWC(IC2,JC2,N2,L2,M2,ALPHA,BETA,GAMMA,PP)
...
!         WUnit(3)= COS(BETA)
      ...
      DO 10 I4=1,NPP
         ...
         WWLM1= WWC(IC1,JC1,N1,L1,M1,ALPHA,BETA,GAMMA,PP)
         WWLM2= VVS(IC2,JC2,N2,L2,M2,ALPHA,BETA,GAMMA,PP)
...
!         WUnit(3)= COS(BETA)
      ...
      DO 10 I4=1,NPP
         ...
         WWLM1= VVS(IC1,JC1,N1,L1,M1,ALPHA,BETA,GAMMA,PP)
         WWLM2= VVS(IC2,JC2,N2,L2,M2,ALPHA,BETA,GAMMA,PP)
...
!local
        ...
        INTEGER :: I1,J1,IJ1,I2,J2,IJ2
        REAL(wp) :: YM
!        INTEGER :: AA((LL+1)**2*NNMAX*NTEN*NTEN)
!        INTEGER :: BB((LL+1)**2*NNMAX*NTEN*NTEN)
        INTEGER :: ICC,ISS
!        REAL(wp) :: YMM1((LL+1)**2*NNMAX*4)
...
!           write(*,*) 'RightPart_Full:MU_W=', MU_W

           YM= RightPart_Term_RE(PROJ,I1,J1,LL,N,L22,M-1,NNMAX,  &
...
!           write(*,*) 'RightPart_Full:MU_V=', MU_V

           YM=RightPart_Term_IM(PROJ,I2,J2,LL,N,L22,M,NNMAX,  &
                PPM,ALPHAM,BETAM,GAMMAM,        &
                NPP,NALPHA,NBETA,NGAMMA,NTEN,PAR)
...
60      CONTINUE   
!           write(*,*) 'RightPart_Full'
!           DO 62 III=1,(LL+1)**2*NNMAX*NTEN*NTEN
!           write(*,*) YMM1
!62         CONTINUE
177   FORMAT(2I3)
      RETURN
...
!--------------------------------------------------
     ...
     FUNCTION RightPart_Term_RE(PROJ,IC,JC,LL,N,L,M,NNMAX,  &
        ...
        INTEGER :: IC,JC,LL,N,L,M,NNMAX,NPP,NTEN !,NPAR
        INTEGER :: NALPHA,NBETA,NGAMMA
        REAL(wp) :: PROJ(NPP*NALPHA*NBETA*NGAMMA)
        REAL(wp) :: PPM(NPP),UPPM(NPP)
        REAL(wp) :: ALPHAM(NALPHA),BETAM(NBETA),GAMMAM(NGAMMA)
...
!---------------------------------------------------
      SUBROUTINE XXMM(LL,NNMAX,XM,NTEN,NDIM)
...
!-------------------------------------------
      ...
      SUBROUTINE  ACOS(L,M,N,AACOS,NDIM)
        IMPLICIT NONE
        INTEGER, PARAMETER  :: wp = kind(1.0d0) !working precision=double 
        INTEGER, PARAMETER  :: sp = kind(1.0) !working precision=single 
        INTEGER :: L,M,N,NDIM
...
!        IF(L .EQ.0 .AND. M .EQ.0) AACOS(6)=1. 
      ...
      END SUBROUTINE ACOS
!----------------------------------------------------------
      SUBROUTINE  ASIN(L,M,N,AASIN,NDIM)
        IMPLICIT NONE
        INTEGER, PARAMETER  :: wp = kind(1.0d0) !working precision=double 
        INTEGER, PARAMETER  :: sp = kind(1.0) !working precision=single 
        INTEGER :: L,M,N,NDIM
