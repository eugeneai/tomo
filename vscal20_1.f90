...
!          GOTO 777
      ...
      DO 81 NII=1,NTEN*NTEN
      DO 81 NJJ=1,NTEN*NTEN    !NII
...
! initial values for the solution
      DO 13 I=1,NTEN*NTEN*LN
         XMVEC0(I)=0.
 13   CONTINUE 

      CALL XYANG_1(AMVEC3,YMVEC3,XMVEC0,NTEN*NTEN*LN,NTEN*NTEN*LN,NITER)
!... am(n1,n2) - matrix 
!....gm(n1) - right part 
...
!      CALL Vec_Line_Inv(XMVEC0,XXM2,LN,NTEN)
      ...
      write(*,117) (XMVEC0(I), I=1, NTEN*NTEN*LN)

      CALL GNUFORM_0 (NTEN*NTEN*LN,XMVEC0,KAN(31)) 
...
!====================================================
      ...
      SUBROUTINE Sort_Matr(AAM,LN,DDM,NTEN,NII,NJJ)
        IMPLICIT NONE
        INTEGER, PARAMETER  :: wp = kind(1.0d0) !working precision=double 
        INTEGER, PARAMETER  :: sp = kind(1.0) !working precision=single 

        INTEGER :: LN,NTEN,NII,NJJ
...
!             NBETA,NGAMMA,NALPHA,NPAR,PAR,VLMINT,NCP1,NCP2)
      ...
      CALL INT_VLMM_TEN(L1,M1,K1,L2,M2,K2,ALPHAM,BETAM,GAMMAM, &
        NBETA,NGAMMA,NALPHA,PAR,VLMINT,NI,NJ,NTEN)
...
! correspond to J=(i',j')
         VLMRE2=WLMM_RE_TEN(L2,M2,K2,ALPHA,BETA,GAMMA) !,NI11,NI22)
         VLMIM2=WLMM_IM_TEN(L2,M2,K2,ALPHA,BETA,GAMMA) !,NJ1,NJ2)

         WU=WUnit(NI1)*WUnit(NI2)*WUnit(NJ1)*WUnit(NJ2)
         
         VLCC=VLMRE1*VLMRE2*WU
         VLCS=VLMRE1*VLMIM2*WU
         VLSC=VLMIM1*VLMRE2*WU
         VLCS=VLMRE1*VLMIM2*WU
         VLSS=VLMIM1*VLMIM2*WU
...
!--------------------------------------------------------
      FUNCTION WLMM_RE_TEN(L,M,M1,ALPHA,BETA,GAMMA) 
...
!----------------------------------------------
      FUNCTION WLMM_IM_TEN(L,M,M1,ALPHA,BETA,GAMMA)
...
!   работает массив UPPM
!   PPM - может быть использован если проекции заданы на неравномерной сетке 
!   например в веерной геометрии  
        USE PARAM_INPUT
...
!      write(*,*) ' INT_Proj_GNLM_P',NALPHA,NBETA,NGAMMA,HUPP
